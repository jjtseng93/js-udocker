...
export async function startCompose(fpath="compose.yaml")
{
  ...
  process.chdir(gb.ctx)
  cse("Changed cwd to:",gb.ctx)
  ...
  for(let svcn in yobj.services)
  {
    ...
    let arr=parseEnvfile(path.join(gb.ctx,".env"));
    arr.forEach(i=>{
      let kv=i.split("=");
    ...
    // select container stage from Dockerfile
    if(svc.build?.target)
    {
      let tgt = svc.build.target ;
      let ctx = svc.build.context ;
      let dcf = path.join(ctx,"Dockerfile")
      
      if( fs.existsSync(dcf) )
      {
        let s=fs.readFileSync(dcf).toString();
        let arr=s.match(/(^|\n)FROM.+(\n|$)/g);
        for(let i in arr)
        {
          let item=arr[arr.length-1-i];

          if(item.includes("AS "+tgt) ||
             item.includes("as "+tgt)  )
          {
            svc.ctnName+=(i||"");
          }
        }
