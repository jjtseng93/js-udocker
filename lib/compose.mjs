...
export async function startCompose(fpath="compose.yaml")
{
  ...
  for(let svcn in yobj.services)
  {
    ...
    if(svc.entrypoint)
    {
      ...
      run_cmd.push("--entrypoint",a.join("\n"))
    }
...
`;
  ...
  for(let rcmd of gb.run_cmds)
  {
    ...
    let r = await rl.question("\x1b[31m ** If the previous service started successfully, please press Enter to continue, or type shell to enter that container **\x1b[0m") ;
...
export function parsetoArray(poss)
{
  let a;
      ...
      {
        cse(`shell-form command/entrypoint is not supported!
