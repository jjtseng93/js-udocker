...
class DockerApi {
  ...
  async get_v2_image_layer(imagerepo, layer_id) {
  ...
  async get_v2(imagerepo, tag, platform) {
    ...
    if (manifest.fsLayers) {
      return this.get_v2_layers_all(imagerepo, [...manifest.fsLayers].reverse());
    }
    if (manifest.layers) {
      const layers = [...manifest.layers];
      if (manifest.config) layers.push(manifest.config);
      return this.get_v2_layers_all(imagerepo, layers);
    }
    Msg.err("Error: layers section missing in manifest");
    return [];
  }

  async get(imagerepo, tag, platform) {
    ...
    const files = await this.get_v2(remoteRepo, tag, platform);
