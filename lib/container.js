...
class ContainerStructure {
  ...
  _untar_layers(tarfiles, destdir) {
    ...
    for (const tarf of tarfiles) {
      ...
      Msg.err("Info: extracting:", tarf, { l: Msg.INF });
  ...
  create_fromimage(imagerepo, tag) {
    const prepared = this.prepare_fromimage(imagerepo, tag);
    if (!prepared) return false;
    const status = this._untar_layers(prepared.layerFiles, prepared.rootfs);
    if (!status) {
  ...
  prepare_fromimage(imagerepo, tag) {
    ...
    return {
      ...
      layerFiles,
      rootfs: path.join(containerDir, "ROOT"),
    };
  }

  apply_whiteouts(tarfile, rootfs) {
    this._apply_whiteouts(tarfile, rootfs);
  }
}
