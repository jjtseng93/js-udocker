...
waiting_for_db(){
  ...
  echo -e "\033[33m Connection established! \033[0m"
  waiting_for_db_ready
  echo -e "\033[33m Database ready! \033[0m"
}
...
create_postgresql_db(){
  echo -e "\033[33m Creating postgresql db \033[0m"
...
if [ ${ONLYOFFICE_DATA_CONTAINER} != "true" ]; then
  ...
  if [ "${PLUGINS_ENABLED}" = "true" ]; then
    ( documentserver-pluginsmanager.sh -r false --update="${APP_DIR}/sdkjs-plugins/plugin-list-default.json" >/dev/null; echo "[pluginsmanager] Plugins initialization finished" >/dev/null ) &
  fi
...
echo ""
echo ""
echo -e "\033[33m Trying to start example server, may fail! \033[0m"
echo "  If not started properly, enter shell and run: sh startdse.sh"
echo ""
echo "  http://localhost:8081"
echo ""
supervisorctl start ds:example
echo "  If you want to shut down, press Ctrl+C*2"
echo "  then use ps -eo pid,args to show database pid"
echo "  then use kill db_pid to stop mariadb"
echo ""
echo ""
echo ""
